...
const QUICK_QUESTIONS = [
  { emoji: '📊', text: 'Bagaimana statistik keuangan bulan ini?' },
  { emoji: '📅', text: 'Kapan laporan harian terakhir?' },
  { emoji: '📦', text: 'Stok bahan apa yang mendekati habis?' },
  { emoji: '⚠️', text: 'Apa yang perlu diperhatikan hari ini?' },
];

// No longer need local getAnswer, since askAssistant handles both dynamic context & static KNOWLEDGE_BASE.
...
export default function GlobalAiAssistant() {
  ...
  // Drag handling (same as AsistenDapur)
  ...
  const send = async (text: string) => {
    ...
    setMessages(prev => [...prev, { role: 'user', content: text.trim(), ts: now }]);
    setInput('');
    setLoading(true);
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: res.jawaban, ts: aiNow }]);
    } catch {
      const errorNow = new Date().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' });
      setMessages(prev => [...prev, { role: 'assistant', content: 'Maaf, terjadi kesalahan. Coba lagi.', ts: errorNow }]);
    }
    setLoading(false);
  };
    ...
    <>
      ...
      {isOpen && (
        <div className="fixed inset-0 sm:inset-auto sm:right-6 sm:bottom-6 z-50 flex flex-col bg-white sm:w-[380px] sm:h-[600px] sm:rounded-2xl shadow-2xl overflow-hidden animate-slide-up sm:border border-slate-200">
          {/* Header */}
          <div className="flex items-center justify-between px-5 py-4" style={{ background: 'linear-gradient(135deg, #1e6fbf, #1a5fa3)', borderBottom: '1px solid #e2e8f0' }}>
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 bg-white/20 rounded-xl flex items-center justify-center">
                <Bot size={20} className="text-white" />
              </div>
              <div>
                <p className="font-bold text-white text-sm">AI Assistant 🤖</p>
                <p className="text-[10px] text-blue-200">Tanya apa saja tentang SPPG Manager</p>
              </div>
            </div>
            ...
            {messages.length === 0 && (
              <div className="space-y-3 pt-4">
                ...
                {QUICK_QUESTIONS.map((q, i) => (
                  <button key={i}
                    onClick={() => send(q.text)}
                    className="w-full text-left px-3.5 py-2.5 bg-white border border-slate-200 rounded-xl text-sm text-slate-700 hover:border-blue-300 hover:bg-blue-50/50 transition-all flex items-center gap-2.5 group"
                  >
            ...
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-fade-in`}>
                <div className="max-w-[85%]">
                  <div className={`px-3.5 py-2.5 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap ${m.role === 'user' ? 'text-white bg-[#1e6fbf] rounded-br-md' : 'text-slate-700 bg-[#f8fafc] border rounded-bl-md'}`}>
                    {m.content}
                  </div>
          ...
          <div className="p-3 border-t bg-white" style={{ borderColor: '#e2e8f0' }}>
            <div className="flex items-end gap-2">
              <textarea
                ...
                rows={1}
                className="flex-1 resize-none text-sm border border-slate-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-400 bg-slate-50"
